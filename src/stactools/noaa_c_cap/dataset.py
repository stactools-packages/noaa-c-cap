...
class Dataset:
    ...
    def from_hrefs(cls, hrefs: List[str]) -> List["Dataset"]:
        ...
        """
        datasets = []
        for tif_href in (href for href in hrefs if os.path.splitext(href)[1] == ".tif"):
            ...
            if dataset.xml_file_name:
                xml_href = next(
                    (
                        href
                        for href in hrefs
                        if os.path.basename(href) == dataset.xml_file_name
                    ),
                    None,
                )
                if xml_href:
                    ...
                    logger.warn(f"Could not find XML metadata file for {tif_href}")
